-- ==============================================================================
-- TeleCaller CRM Enterprise — Phase 4.3.1 Security Hardening Migration
-- SECURITY DEFINER Search Path Isolation & RPC Role Privileges
-- ==============================================================================

-- 1. ATOMIC LEAD ASSIGNMENT (HARDENED)
...
-- 2. ATOMIC ROUND-ROBIN AUTO-DISTRIBUTION (HARDENED)
...
-- 3. ATOMIC CALL LOGGING & STATUS UPDATE (HARDENED)
...
-- 4. ATOMIC FOLLOW-UP CREATION (HARDENED)
...
-- 5. ATOMIC FOLLOW-UP COMPLETION (HARDENED WITH IDOR PROTECTION)
...
-- 6. ATOMIC TELECALLER DELETION (HARDENED)
...
DECLARE
  ...
  v_now TIMESTAMPTZ := NOW();
BEGIN
  SELECT name, login_id INTO v_tc_name, v_tc_login FROM public.users WHERE organization_id = p_org_id AND id = p_telecaller_id;
  IF v_tc_name IS NULL THEN
  ...
  FOR v_lead_rec IN
    SELECT id FROM public.leads WHERE organization_id = p_org_id AND assigned_to = p_telecaller_id FOR UPDATE
  LOOP
    UPDATE public.leads SET assigned_to = NULL, updated_at = v_now WHERE id = v_lead_rec.id;
  ...
  DELETE FROM public.users WHERE organization_id = p_org_id AND id = p_telecaller_id;
...
-- ==============================================================================
-- 7. EXECUTE PRIVILEGES HARDENING
-- Revoke public execution of sensitive security functions & restrict to service_role
-- ==============================================================================
...
REVOKE EXECUTE ON FUNCTION delete_telecaller_atomic(VARCHAR, VARCHAR, VARCHAR) FROM PUBLIC, anon, authenticated;
...
GRANT EXECUTE ON FUNCTION delete_telecaller_atomic(VARCHAR, VARCHAR, VARCHAR) TO service_role;
