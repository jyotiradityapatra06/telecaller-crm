...
export const HrDashboard: React.FC<{ user: AuthUser; leads: Lead[]; telecallers: AuthUser[]; onNavigate: (tab: string) => void }> = ({ user, leads, telecallers, onNavigate }) => {
  ...
  return <div className="max-w-6xl mx-auto p-5 sm:p-8 space-y-8">
    ...
    <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">{[
      ['Telecallers', telecallers.length, <Users/>], ['Active Worklist Items', telecallers.reduce((sum, tc) => sum + (tc.metrics?.assignedLeads || 0), 0), <Users/>], ['Unique Contacts', leads.length, <Users/>], ['Calls Today', telecallers.reduce((sum, tc) => sum + (tc.metrics?.callsMade || 0), 0), <Phone/>], ['Follow-ups', telecallers.reduce((sum, tc) => sum + (tc.metrics?.followUps || 0), 0), <Clock/>]
    ...
    <section className="bg-white border border-slate-200 rounded-2xl overflow-hidden"><div className="px-5 py-4 border-b"><h2 className="font-bold text-slate-900">Telecaller progress</h2></div><div className="divide-y">{telecallers.map(tc=>{const metrics=tc.metrics;return <button key={tc.id} onClick={()=>onNavigate('telecallers')} className="w-full grid grid-cols-2 sm:grid-cols-4 gap-2 text-left px-5 py-4 hover:bg-slate-50"><span className="font-semibold text-slate-900">{tc.name}</span><span>{metrics?.assignedLeads || 0} leads</span><span>{metrics?.callsMade || 0} calls today</span><span>{metrics?.followUps || 0} follow-ups</span></button>})}{telecallers.length===0&&<p className="p-8 text-center text-slate-500">Create your first telecaller to start assigning leads.</p>}</div></section>
  </div>;
};
