...
export const HrTelecallers: React.FC<{ user: AuthUser; telecallers: AuthUser[]; leads: Lead[]; refresh: () => Promise<boolean> }> = ({ user, telecallers, leads, refresh }) => {
  ...
  return <div className="max-w-6xl mx-auto p-5 sm:p-8 space-y-6">
    ...
    <div className="bg-white border rounded-2xl overflow-x-auto"><table className="w-full min-w-[820px] text-sm"><thead className="bg-slate-50 text-left"><tr>{['Name', 'Login ID', 'Leads', 'Called', 'Pending', 'Follow-ups', 'Status', 'Actions'].map((heading) => <th key={heading} className="p-4">{heading}</th>)}</tr></thead><tbody className="divide-y">{telecallers.map((telecaller) => { const own = leads.filter((lead) => lead.assignedTo === telecaller.id); const called = own.filter((lead) => lead.totalCallsCount > 0).length; return <tr key={telecaller.id}><td className="p-4 font-semibold">{telecaller.name}</td><td className="p-4">{telecaller.loginId}</td><td className="p-4">{own.length}</td><td className="p-4">{called}</td><td className="p-4">{own.length - called}</td><td className="p-4">{own.filter((lead) => lead.activeFollowUp).length}</td><td className={`p-4 ${telecaller.isActive ? 'text-emerald-700' : 'text-rose-700'}`}>{telecaller.isActive ? 'Active' : 'Inactive'}</td><td className="p-4"><div className="flex gap-2"><button disabled={Boolean(busy)} title="Reset password" onClick={() => setResetUser(telecaller)} className="border rounded-lg p-2 disabled:opacity-40"><KeyRound className="w-4" /></button><button disabled={Boolean(busy)} title={telecaller.isActive ? 'Deactivate' : 'Activate'} onClick={() => toggle(telecaller)} className="border rounded-lg p-2 disabled:opacity-40"><Power className="w-4" /></button></div></td></tr>; })}{telecallers.length === 0 && <tr><td colSpan={8} className="p-8 text-center text-slate-500">No telecallers yet. Create your first telecaller to start assigning leads.</td></tr>}</tbody></table></div>
